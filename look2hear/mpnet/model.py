...
import numpy as np
from look2hear.mpnet.transformer import TransformerBlock
from look2hear.utils.mpnet_utils import LearnableSigmoid2d
...
class MPNet(nn.Module):
    def __init__(self, h, num_tsblocks=2):
    ...
    def forward(self, noisy_amp, noisy_pha): # [B, F=201, T=321]

        x = torch.stack((noisy_amp, noisy_pha), dim=-1).permute(0, 3, 2, 1) # [B, 2, T=321, F=201]
        x = self.dense_encoder(x) #(4,64,321,101)

        for i in range(self.num_tscblocks): #4번
            x = self.TSTransformer[i](x) #(1,64,321,101)
