...
class MultiHeadSelfAttentionSuppressor2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        attn_mat =0.75*F.softmax(attn_mat, dim=2)+0.25*F.softmax(-attn_mat_noise, dim=2)  # [B', T, T]
...
class TIGERALL(BaseModel):
    def __init__(
        ...
        super(TIGERALL, self).__init__(sample_rate=sample_rate)
