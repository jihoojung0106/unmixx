...
class TIGERALL2(BaseModel):
    def __init__(
        ...
        super(TIGERALL2, self).__init__(sample_rate=sample_rate)
    ...
    def do_istft(self, spec, nsample):
        ...
        return output
    def get_mag_pha_from_stft(self, stft_spec):
        stft_spec = torch.view_as_real(stft_spec)
        mag = torch.sqrt(stft_spec.pow(2).sum(-1)+(1e-9))
        mag = torch.pow(mag, self.compress_factor)
        pha = torch.atan2(stft_spec[:, :, :, 1]+(1e-10), stft_spec[:, :, :, 0]+(1e-5))
        return mag, pha
    def mag_pha_stft(self,input):
        ...
        return mag, pha, com
    # def mag_pha_istft(self,spec,mag, pha,nsample):
    ...
    def forward(self, input,istest=False):
        ...
        mag1,phase1=self.get_mag_pha_from_stft(sep_subband_spec[:,0])
        mag2,phase2=self.get_mag_pha_from_stft(sep_subband_spec[:,1])
        return reconstructed,mag1,phase1,mag2,phase2
