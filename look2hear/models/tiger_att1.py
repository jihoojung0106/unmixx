...
class Recurrent(nn.Module):
    def __init__(
        ...
        self.spk_attention=MySpeakerAttention(out_channels//2, 1, n_head=n_head, hid_chan=att_hid_chan, act_type="prelu", norm_type="LayerNormalization4D", dim=4)
    ...
    def forward(self, x):
        ...
        for i in range(self.iter):
            ...
            x= self.apply_spk_attention(x, N) # B, nband*2, N/2, T  
        return x.permute(0, 2, 1, 3).contiguous() # B, nband, N, T
    def apply_spk_attention(self, x, N): #B, N, nband, T
        x1 = x[:, :N // 2, :, :]  #B, N//2, nband, T
        x2 = x[:, N // 2:, :, :]#B, N//2, nband, T
        x1,x2=self.spk_attention(x1,x2) #x: (B, N//2, nband, T)->[B, N//2, nband, T]
        return torch.cat([x1, x2], dim=1)         # (B, N, nband, T)
...
class TIGERATT1(BaseModel):
    def __init__(
        ...
        super(TIGERATT1, self).__init__(sample_rate=sample_rate)
