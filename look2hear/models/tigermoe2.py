...
class TIGERMOE2(BaseModel):
    def __init__(
        ...
        super(TIGERMOE2, self).__init__(sample_rate=sample_rate)
        ...
        self.separator2 = Recurrent(self.feature_dim, in_channels, self.nband, upsampling_depth, att_n_head, att_hid_chan, att_kernel_size, att_stride, 2)       
    ...
    def do_istft(self, spec, nsample):
        ...
        return output
    def reconstruct_from_sep_output(self,input,sep_output1, subband_spec):
        if input.ndim == 1:
            was_one_d = True
        ...
        sep_subband_spec = []
        for i in range(self.nband):
            this_output = self.mask[i](sep_output1[:,i]).view(batch_size*nch, 2, 2, self.num_output, self.band_width[i], -1)
    ...
    def forward(self, input,inter=False,istest=False):
        ...
        # import pdb; pdb.set_trace()
        # separator
        sep_output1 = self.separator(subband_feature.view(batch_size*nch, self.nband, self.feature_dim, -1))  # B, nband, N, T
        sep_output2 = self.separator2(sep_output1)
        sep_output1 = sep_output1.view(batch_size*nch, self.nband, self.feature_dim, -1) #(1,67,192,401)
        sep_subband_spec = []
        for i in range(self.nband):
            this_output = self.mask[i](sep_output1[:,i]).view(batch_size*nch, 2, 2, self.num_output, self.band_width[i], -1)
            this_mask = this_output[:,0] * torch.sigmoid(this_output[:,1])  # B*nch, 2, K, BW, T
            ...
            est_spec_imag = subband_spec[i].real.unsqueeze(1) * this_mask_imag + subband_spec[i].imag.unsqueeze(1) * this_mask_real  # B*nch, K, BW, T
            sep_subband_spec.append(torch.complex(est_spec_real, est_spec_imag))
        sep_subband_spec1 = torch.cat(sep_subband_spec, 2) #(1,2,321,401)
        
        output = torch.istft(sep_subband_spec1.view(batch_size*nch*self.num_output, self.enc_dim, -1), 
                             n_fft=self.win, hop_length=self.stride,
                             window=torch.hann_window(self.win).to(input.device).type(input.type()), length=nsample)
        b  = output.view(batch_size*nch, self.num_output, -1)
        reconstructed = pad_x_to_y(output, input)
        ...
        sep_subband_spec = []
        for i in range(self.nband):
            this_output = self.mask[i](sep_output2[:,i]).view(batch_size*nch, 2, 2, self.num_output, self.band_width[i], -1)
            this_mask = this_output[:,0] * torch.sigmoid(this_output[:,1])  # B*nch, 2, K, BW, T
            this_mask_real = this_mask[:,0]  # B*nch, K, BW, T
            this_mask_imag = this_mask[:,1]  # B*nch, K, BW, T
            # force mask sum to 1
            ...
            est_spec_imag = subband_spec[i].real.unsqueeze(1) * this_mask_imag + subband_spec[i].imag.unsqueeze(1) * this_mask_real  # B*nch, K, BW, T
            sep_subband_spec.append(torch.complex(est_spec_real, est_spec_imag))
        sep_subband_spec2 = torch.cat(sep_subband_spec, 2) #(1,2,321,401)
        
        output = torch.istft(sep_subband_spec2.view(batch_size*nch*self.num_output, self.enc_dim, -1), 
                             n_fft=self.win, hop_length=self.stride,
                             window=torch.hann_window(self.win).to(input.device).type(input.type()), length=nsample)
        b  = output.view(batch_size*nch, self.num_output, -1)
        reconstructed2 = pad_x_to_y(output, input)
        reconstructed2 = reconstructed2 + 1 / reconstructed2.shape[1] * (
            input.unsqueeze(1) - reconstructed2.sum(dim=1, keepdim=True)
        )
        return reconstructed,reconstructed2,sep_output1,sep_output2,sep_subband_spec1,sep_subband_spec2,subband_spec
