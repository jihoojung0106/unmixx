...
class SpkAttention(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, num_spk: int=2):
        """
        Compute encoded features.
            :param torch.Tensor x: encoded source features (batch, max_time_in, size)
            :param torch.Tensor mask: mask for x (batch, max_time_in)
...
class MultiHeadSelfAttention2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return x
class MultiHeadSelfAttentionSuppressor2D(nn.Module):
    def __init__(
        self,
        in_chan: int,
        n_freqs: int,
        ...
        **kwargs,
        ...
        super(MultiHeadSelfAttentionSuppressor2D, self).__init__()
        self.in_chan = in_chan
        self.n_freqs = n_freqs
        self.n_head = n_head
        self.hid_chan = hid_chan
        ...
        self.Keys = nn.ModuleList()
        self.Values = nn.ModuleList()
        self.Queries_noise = nn.ModuleList()
        for _ in range(self.n_head):
            self.Queries.append(
                ATTConvActNorm(
                    in_chan=self.in_chan,
    ...
    def forward(self, x: torch.Tensor):
        ...
        batch_size, channel, time, freq = x.size()
        x_noise = torch.cat([x[:, channel//2:, :, :], x[:, :channel//2, :, :]], dim=1)
        residual = x

        all_Q = [q(x) for q in self.Queries]  # [B, E, T, F]
        all_Q_noise = [q(x_noise) for q in self.Queries_noise]
        all_K = [k(x) for k in self.Keys]  # [B, E, T, F]
        all_V = [v(x) for v in self.Values]  # [B, C/n_head, T, F]

        Q = torch.cat(all_Q, dim=0)  # [B', E, T, F]    B' = B*n_head
        Q_noise = torch.cat(all_Q_noise, dim=0)  # [B', E, T, F]    B' = B*n_head
        K = torch.cat(all_K, dim=0)  # [B', E, T, F]
        V = torch.cat(all_V, dim=0)  # [B', C/n_head, T, F]

        Q = Q.transpose(1, 2).flatten(start_dim=2)  # [B', T, E*F]
        Q_noise = Q_noise.transpose(1, 2).flatten(start_dim=2)  # [B', T, E*F]
        K = K.transpose(1, 2).flatten(start_dim=2)  # [B', T, E*F]
        V = V.transpose(1, 2)  # [B', T, C/n_head, F]
        old_shape = V.shape
        V = V.flatten(start_dim=2)  # [B', T, C*F/n_head]
        ...
        attn_mat = torch.matmul(Q, K.transpose(1, 2)) / (emb_dim**0.5)  # [B', T, T]
        attn_mat_noise = torch.matmul(Q_noise, K.transpose(1, 2)) / (emb_dim**0.5)
        attn_mat =0.9*F.softmax(attn_mat, dim=2)+0.1*F.softmax(-attn_mat_noise, dim=2)  # [B', T, T]
        V = torch.matmul(attn_mat, V)  # [B', T, C*F/n_head]
        V = V.reshape(old_shape)  # [B', T, C/n_head, F]
        V = V.transpose(1, 2)  # [B', C/n_head, T, F]
        emb_dim = V.shape[1]  # C/n_head
...
class MySpeakerAttention(nn.Module):
    def __init__(
        self,
        in_chan: int,
        n_freqs: int,
        ...
        **kwargs,
        ...
        super(MySpeakerAttention, self).__init__()
        self.in_chan = in_chan
        self.n_freqs = n_freqs
        self.n_head = n_head
        self.hid_chan = hid_chan
    ...
    def forward(self, x1: torch.Tensor,x2: torch.Tensor):
        '''x: (B, N//2, nband, T)'''
        if self.dim == 4: #B, N//2, nband, T
            x1 = x1.transpose(-2, -1).contiguous()#(batch,128,67,401)
            x2 = x2.transpose(-2, -1).contiguous()
        batch_size, ch, time, freq = x1.size()#(batch,128,401,67)
        residual1 = x1
        residual2 = x2
        x=torch.cat([x1.unsqueeze(1),x2.unsqueeze(1)],dim=1) # (batch,2,128,401,67)
        x=x.view(2*batch_size, ch, time, freq).contiguous()  # [B*2, C, T, F]    B' = B*n_head
        all_Q = [q(x) for q in self.Queries]  # [B*2, E, T, F]
        all_K = [k(x) for k in self.Keys]  # [B*2, E, T, F]
        all_V = [v(x) for v in self.Values]  # [B*2, C/n_head, T, F]
        ...
        Q=Q.view(batch_size*time*self.n_head, 2, self.hid_chan, freq).contiguous()  # [B*t, 2, E, F]
...
class Recurrent(nn.Module):
    def __init__(
        ...
        self.freq_path = nn.ModuleList([
            ...
            MultiHeadSelfAttentionSuppressor2D(out_channels, 1, n_head=n_head, hid_chan=att_hid_chan, act_type="prelu", norm_type="LayerNormalization4D", dim=4),
        ...
        #self.spk_attention = nn.ModuleList()
        #self.attention=SpkAttention(64,num_mha_heads=8, dropout_rate=0.1)
    ...
    def forward(self, x):
        ...
        for i in range(self.iter):
            ...
            #x= self.apply_spk_attention(x, B, N, nband, T) # B, nband*2, N/2, T  
        return x.permute(0, 2, 1, 3).contiguous() # B, nband, N, T
    def apply_spk_attention(self, x, B, N, nband, T): #B, F, T
        'x: B, N, nband, T'
        x1=x[:,:N//2,:,:] # B, N/2, nband, T
        x2=x[:,N//2:,:,:] # B, N/2, nband, T 
        x1=x1.permute(2,0,1,3).contiguous() # B, nband, N/2, T -> nband, B, N/2, T
        x2=x2.permute(2,0,1,3).contiguous() # B, nband, N/2, T -> nband, B, N/2, T
        x= torch.cat([x1.unsqueeze(1), x2.unsqueeze(1)], dim=1) # nband, B, 2, N/2, T
        x=x.view(nband,B*2,N//2,T).contiguous() # nband, B*2, N/2, T
        x_cat = x.view(nband * B * 2, N // 2, T)  # (nband*B*2), N/2, T
        out = self.attention(x_cat)  # (nband*B*2), N/2, T
        out = out.view(nband, B * 2, N // 2, T)
        out1 = out[0::1][:, 0::2]  # nband, B, N/2, T
        out2 = out[0::1][:, 1::2]  # nband, B, N/2, T
        out1= out1.permute(1, 2, 0, 3).contiguous() # B, N/2, nband, T
        out2= out2.permute(1, 2, 0, 3).contiguous()
...
class TIGERATT4(BaseModel):
    def __init__(
        ...
        super(TIGERATT4, self).__init__(sample_rate=sample_rate)
        ...
        bandwidth_1k = int(np.floor(1000 / (sample_rate / 2.) * self.enc_dim))
        ...
        self.band_width += [bandwidth_1k] * 3
        self.band_width.append(self.enc_dim - np.sum(self.band_width))
        self.nband = len(self.band_width)
        print(self.band_width,len(self.band_width), self.nband)
    ...
    def do_stft(self,input):
        ...
        return spec
    def do_istft(self, spec, nsample):
        output = torch.istft(spec, n_fft=self.win, hop_length=self.stride,
                             window=torch.hann_window(self.win).to(spec.device).type(spec.type()), length=nsample)
        return output
    def forward(self, input,inter=False,istest=False):
        # input shape: (B, C, T)
        was_one_d = False
        if input.ndim == 1:
            was_one_d = True
        ...
        sep_output = sep_output.view(batch_size*nch, self.nband, self.feature_dim, -1) #(1,67,192,401)
        ...
        sep_subband_spec = torch.cat(sep_subband_spec, 2) #(1,2,321,401)
        ...
        b  = output.view(batch_size*nch, self.num_output, -1)
        reconstructed = pad_x_to_y(output, input)
        reconstructed = reconstructed + 1 / reconstructed.shape[1] * (
            input.unsqueeze(1) - reconstructed.sum(dim=1, keepdim=True)
        )
        if inter:
            return reconstructed,sep_output,subband_spec ##(1,67,192,401)
