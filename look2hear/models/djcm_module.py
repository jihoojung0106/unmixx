...
class DJCMEncoder(nn.Module):
    def __init__(self, in_channels, n_blocks):
        ...
        self.en_blocks = nn.ModuleList([
            ...
            EncoderBlock(64, 64, n_blocks, (1, 2), False),
...
class Decoder(nn.Module):
    def __init__(self, n_blocks, gate=False):
        ...
        self.de_blocks = nn.ModuleList([
            ...
            DecoderBlock(64, 64, n_blocks, (1, 2), False, gate),
            DecoderBlock(64, 64, n_blocks, (1, 2), False, gate),
...
class DJCMLatentBlocks(nn.Module):
    def __init__(self, n_blocks, latent_layers):
        ...
        for i in range(latent_layers):
            self.latent_blocks.append(EncoderBlock(64, 64, n_blocks, None, False))
...
class DJCMPE_Decoder(nn.Module):
    def __init__(self, n_blocks, seq_frames, seq='gru', seq_layers=1, gate=False):
        ...
        if seq.lower() == 'gru':
            ...
            self.fc = nn.Sequential(
                BiLSTM((seq_frames, FREQ_DIM), (1, FREQ_DIM), 1, seq_layers), #<-mamba로 교체?
                nn.Linear(FREQ_DIM, N_CLASS),
                nn.Sigmoid()
            )
        else:
    ...
    def forward(self, x, concat_tensors,mixed_salience=None):
        x = self.de_blocks(x, concat_tensors,mixed_salience) #(1,128,401,40) (32->64->128)
...
class DecoderCrossAttention(nn.Module):
    def __init__(self, n_blocks, gate=False):
        ...
        self.cross_attention.append(MyCrossAttention(64))
        self.cross_attention.append(MyCrossAttention(64))
