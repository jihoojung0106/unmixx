...
class DilatedConvNorm(nn.Module):
    ...
    def forward(self, input):
        output = self.conv(input)
        return self.norm(output)
class DilatedDeconvNorm(nn.Module):
    """
    This class defines the transposed convolution (deconvolution) with normalization,
    used for learnable upsampling to mirror DilatedConvNorm.
    """
    def __init__(self, nIn, nOut, kSize=5, stride=4, d=1, groups=64):
        super().__init__()
        padding = ((kSize - 1) // 2) * d
...
class UConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        residual = x.clone() #x:(401,128=d,67=nband)
        # Reduce --> project high-dimensional feature maps to low-dimensional space
        output1 = self.proj_1x1(x) #(401,256=out_channels,67=nband)
        output = [self.spp_dw[0](output1)] #[(401,256=out_channels,67=nband)]

        # Do the downsampling process from the previous level
        for k in range(1, self.depth): #5
            out_k = self.spp_dw[k](output[-1]) #(401,256=out_channels,34=nband)...->(401,256=out_channels,5=nband)
            output.append(out_k)

        # global features
        global_f = torch.zeros(
            output[-1].shape, requires_grad=True, device=output1.device
        ) # (401,256=out_channels,5=nband)
        for fea in output:
            global_f = global_f + F.adaptive_avg_pool1d(
                fea, output_size=output[-1].shape[-1]
            )
            # global_f = global_f + fea
        global_f = self.globalatt(global_f)  # (401,256=out_channels,5=nband)
        ...
        # import pdb; pdb.set_trace() expanded ,residual : (401,256=out_channels,67=nband)
...
class Recurrent(nn.Module):
    def __init__(
        ...
        for i in range(self.nband):
            self.spk_attention.append(
                SpkAttention(64,num_mha_heads=8, dropout_rate=0.1)  # 64 is the output channels of UConvBlock
            )
        
    def forward(self, x):
        # B, nband, N, T
        B, nband, N, T = x.shape
        x = x.permute(0, 2, 1, 3).contiguous() # B, N=128, nband=67, T
        mixture = x.clone()
    ...
    def apply_spk_attention(self, x, B, N, nband, T): #B, F, T
        ...
        out_list2=[]
        for i in range(self.nband):
            out = self.spk_attention[i](x[i]) #B*2, N/2, T
            out_list1.append(out[0::2])
            out_list2.append(out[1::2])
        out1 = torch.stack(out_list1, dim=0) # nband, B, N/2, T
        out2= torch.stack(out_list2, dim=0) # nband, B, N/2, T
...
class MyTIGERSPK3(BaseModel):
    def __init__(
        ...
        stride=512,
        num_sources=2,
        sample_rate=44100,is_mine=False
    ):
        super(MyTIGERSPK3, self).__init__(sample_rate=sample_rate)
        num_blocks=8
    ...
    def forward(self, input,target_wav=None):
        # input shape: (B, C, T)
        was_one_d = False
        if input.ndim == 1:
            was_one_d = True
        ...
        if self.is_mine:
            return output,subband_spec,sep_output
        outdict={
            #"first_est_speech":output[:,0],
            "target_speech":target_wav,
            "second_est_speech":output
        }
        return outdict
