...
def change_pitch_and_formant_random(audio, sample_rate):
    ...
    pitch_shift_ratio = random.uniform(-0.12, 0.12)  # -15 to +15 cents
    pitch_shift_ratio = random.choice([-6, 0, 6]) + pitch_shift_ratio

    sound = change_pitch(sound, pitch_shift_ratio)  # -15 to +15 cents

    formant_shift_ratio = random.uniform(1, 1.2)
