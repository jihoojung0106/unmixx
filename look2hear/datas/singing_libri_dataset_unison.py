...
def save_waveform(waveform, path, sample_rate=24000):
    """
    Save a waveform to a file.
    
    Args:
        waveform (np.ndarray or torch.Tensor): The audio waveform to save.
        path (str): The file path where the waveform will be saved.
...
class DuetSingingSpeechMixTraining(Dataset):
    ...
    def __init__(
        ...
        self.unison_jsons=glob.glob("duet_svs/24k/NUS/*_unison.json")+glob.glob("duet_svs/24k/CSD/*_unison.json")+glob.glob("duet_svs/24k/k_multitimbre/*_unison.json")
    def __len__(self):
        debug=True
        if debug:
            return 20
    ...
    def load_first_audio(self, rand_prob, paths_info_dict):
        ...
        # Other Cases
            ...
            return source_1, paths_info_dict
    def get_two_random_segments(self,json_path, max_attempts=5):
        with open(json_path, "r") as f:
            data = json.load(f)
        keys = list(data["segment"].keys())
        for _ in range(max_attempts):
            key = random.choice(keys)
            segment_list = data["segment"][key]
            if len(segment_list) >= 2:
                selected_segments = random.sample(segment_list, 2)
                #print(selected_segments,json_path.replace('_unison.json','.wav'))
                start1 = selected_segments[0]["start_time_sec"]
                end1 = selected_segments[0]["end_time_sec"]
                start2 = selected_segments[1]["start_time_sec"]
                end2 = selected_segments[1]["end_time_sec"]
    ...
    def load_second_audio(self, source_1, rand_prob, augment_prob, paths_info_dict):
        ...
        if (augment_prob <= self.unison_prob) and (
            ...
            if random.random() <= 0.2:
                ...
                #unison case
                picked_json_path = random.choice(self.unison_jsons)
                #print()
                picked_item=self.get_two_random_segments(picked_json_path)
                if picked_item is None:
                    source_2 = change_pitch_and_formant_random(source_1, self.sample_rate)
                else:
                    try:
                        start1, end1, start2, end2 = picked_item
                        source_1 = load_wav_specific_position_mono(
                            picked_json_path.replace('_unison.json','.wav'),
                            self.sample_rate, #filename, sample_rate, seq_duration, start_position
                            self.segment,
                            start1,
                        )
                        ...
                        #print(f"Loaded unison segments from {picked_json_path} with start times {start1} and {start2}")
                    except Exception as e:
                        print(f"Error in loading unison segments: {e}")
                        source_2 = change_pitch_and_formant_random(source_1, self.sample_rate)
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if rms_src1<0.001 or rms_src2<0.001:
                    ...
                    visualize=False
                    if visualize:
                        save_folder=f"result/unison_case_test/k_multisinger/{generate_random_string(5)}"
                        os.makedirs(save_folder, exist_ok=True)
                        save_waveform(mix, os.path.join(save_folder,"mix.wav"), self.sample_rate)
                        save_waveform(sources[0], os.path.join(save_folder,"src1.wav"), self.sample_rate)
                        save_waveform(sources[1], os.path.join(save_folder,"src2.wav"), self.sample_rate)
                        #print(f"idx: {idx}, rms_mix: {rms_mix:.4f}, rms_src1: {rms_src1:.4f}, rms_src2: {rms_src2:.4f}")
                    return mix, sources, path
            except Exception as e:
                print(f"[Attempt {attempt+1}/{max_attempts}] Error in getitem({idx}): {e}")
        dummy_mix = torch.zeros((self.sample_rate * self.segment,), dtype=torch.float32)  # 예: 3초짜리 0 waveform
        dummy_sources = torch.zeros((2, self.sample_rate * self.segment), dtype=torch.float32)  # n_src=2 기준
...
class SingingLibriDataModuleUnison(LightningDataModule):
    def __init__(
        self,
        train_dir: str,
        valid_dir: str,
...
if __name__ == "__main__":
    # Example usage
    dm = DuetSingingSpeechMixTraining(
        sing_sing_ratio=1.0, #0.2
        sing_speech_ratio=0.0, #0.2
        same_song_ratio=0.0,
        same_singer_ratio=0.0,
        same_speaker_ratio=0.0, #0.15
        unison_prob=1.0,
    )
    for i in dm:
        print()