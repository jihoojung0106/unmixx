...
def compute_beat_time(y,sr=24000):
    tempo, beats = librosa.beat.beat_track(y=y, sr=sr)
    # beats: 각 비트의 프레임 인덱스  
    beat_times = librosa.frames_to_time(beats, sr=sr)  # 비트 시퀀스(초 단위)
    return beat_times
class DuetSingingSpeechMixTraining(Dataset):
    ...
    def __init__(
        ...
        sing_sing_ratio=0.3, #0.2
        sing_speech_ratio=0.0, #0.2
        same_song_ratio=0.4,
        same_singer_ratio=0.3,
        same_speaker_ratio=0, #0.15
        batch_size=1,
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if rms_src1<0.001 or rms_src2<0.001:
                    ...
                    visualize=False
                    if visualize:
                        save_folder=f"result/harmony/{path}_{harmonic_score}_{generate_random_string(3)}"
                        os.makedirs(save_folder, exist_ok=True)
                        save_audio(mix, f"{save_folder}/mixture.wav", self.sample_rate)
                        print(f"{save_folder}/mixture.wav")
                        # compute_beat_time(sources[0])
                        # compute_beat_time(sources[1])
                    return mix, sources, harmonic_score, category
...
class SingingLibriDataModuleSongOnly(LightningDataModule):
    def __init__(
        self,
        train_dir: str,
        valid_dir: str,
        ...
        sing_sing_ratio=0.4, #0.2
        sing_speech_ratio=0.0, #0.2
        same_song_ratio=0.3,
        same_singer_ratio=0.0,
        same_speaker_ratio=0.3, #0.15
        unison_prob=0.3,pitch_formant_augment_prob=0.4,
        reduced_training_data_ratio=0.03
    ) -> None:
        super().__init__()
