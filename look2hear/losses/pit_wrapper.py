...
class PITLossWrapper(nn.Module):
    ...
    def no_pit_forward(self, ests, targets, return_ests=False, reduce_kwargs=None, return_indices=False, **kwargs):
        """
        PIT loss wrapper without permutation invariant training.
        """
        assert ests.shape == targets.shape, "Estimates and targets must have the same shape."
        pw_loss=self.loss_func(ests, targets, **kwargs)
        ordered_loss = (pw_loss[:, 0, 0] + pw_loss[:, 1, 1])/2
        #mean_loss = ordered_loss.mean()
        return ordered_loss
    def forward(self, ests, targets, return_ests=False, reduce_kwargs=None, return_indices=False, before_mean=False,**kwargs):
        ...
        if self.pit_from == "pw_mtx_multidecoder_keepmtx":
            ...
            if before_mean and not return_ests and not return_indices:
                return min_loss
            elif before_mean and return_ests and not return_indices:
                return min_loss, reordered
            elif before_mean and return_indices:
                return min_loss, reordered, batch_indices
            if not return_ests and not return_indices:
                return mean_loss#,batch_indices
            if not return_ests and return_indices:
                return mean_loss, batch_indices
