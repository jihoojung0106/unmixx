...
class PairwiseNegSDR(_Loss):
    ...
    def forward(self, ests, targets,istest=False):
        ...
        s_target = torch.unsqueeze(targets, dim=1) #( [batch, 1, n_src, time])
        s_estimate = torch.unsqueeze(ests, dim=2) # ( [batch, n_src, 1, time])
