...
class AudioLightningModuleDynamic1(pl.LightningModule):
    def __init__(
        self,
        ...
        isfirst=True
        if isfirst:
            resume = torch.load("Experiments/checkpoint/fin_van1/20250716_23 copy/epoch=100.ckpt")
            epoch_= resume["epoch"]
    ...
    def training_step(self, batch, batch_nb):
        ...
        device_losses = self.loss_func["train"](est_sources, targets,before_mean=True) #(B,)
        loss= torch.mean(device_losses)
        chunked_mixtures = chunk_waveform(mixtures) #(4,36000)
        ...
        device_losses = self.loss_func["train"](est_sources, targets,before_mean=True) #(B,)
        gathered_losses = self.all_gather(device_losses.detach())  # shape: (world_size=gpu개수, B)
        ...
        with torch.no_grad():
            ...
            normalized = normalize_losses(gathered_losses.view(-1), l_min=min_loss, l_max=max_loss)
            reweighted = apply_strategy(normalized, strategy="linupper")
            # Temperature scheduler
            #r = self.r_scheduler(self.global_step)
            scaled = scale_losses(reweighted - reweighted.max().item(),r=3) #(1,0.9)
            weights = scaled / scaled.sum()
            # 현재 GPU에 해당하는 weight 추출
            local_rank = self.global_rank
            device_weights = weights.view(self.trainer.world_size, -1)[local_rank, :]
        loss_metric = (self.bce_loss(metric_g1.flatten(), one_labels)+
                       self.bce_loss(metric_g2.flatten(), one_labels))/2
        loss_metric=torch.sum(device_weights * loss_metric) * self.trainer.world_size
        loss_gen_all= loss + loss_metric*0.5
        optimizer.zero_grad()
    ...
    def validation_step(self, batch, batch_nb, dataloader_idx):
        ...
        if (self.trainer.current_epoch) % 1 == 0 and dataloader_idx >= 1:
            ...
            chunk_batch_size = chunked_mixtures.shape[0]
            one_labels = torch.ones(chunk_batch_size).to(mixtures.device, non_blocking=True)
            chunked_est_sources_final1 = chunk_waveform(est_sources[:,0])
            chunked_est_sources_final2 = chunk_waveform(est_sources[:,1])
