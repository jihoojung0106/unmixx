...
from look2hear.losses.ssnr_loss import compute_ssnr,pit_segmental_ssnr_loss
...
class AudioLightningModuleAll3(pl.LightningModule):
    def __init__(
        self,
        audio_model=None,
        video_model=None,
        ...
        if isfirst:
            ...
            epoch_= resume["epoch"]
            self.my_epoch_start= epoch_ + 200 # Set the current epoch to the loaded epoch
    ...
    def penalty_loss_scheduler(self):
        ...
        if self.current_epoch+self.my_epoch_start < 150:
            return 0.0
        else:
            return 0.002
    ...
    def training_step(self, batch, batch_nb):
        ...
        ssnr_loss=pit_segmental_ssnr_loss(
                                targets[:,0],est_sources[:,0], 
                                targets[:,1],est_sources[:,1] 
                                )
        self.log(
            "train_loss_mag",
            mag_loss,
        ...
        return {"loss": loss + mag_loss*0.1+penalty_loss*penalty_loss_weight+ ssnr_loss*0.1}
    ...
    def validation_step(self, batch, batch_nb, dataloader_idx):
        ...
        if (self.trainer.current_epoch) % 1 == 0 and dataloader_idx >= 1:
            ...
            self.log(
                f"test_ssnr_loss_{dataloader_idx}",
                ssnr_loss,
                on_epoch=True,
                prog_bar=True,
                sync_dist=True,
