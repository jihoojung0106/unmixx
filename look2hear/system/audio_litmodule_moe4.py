...
def expand_gate_to_time(gate: torch.Tensor, total_t: int = 401) -> torch.Tensor:
    ...
    base = 100  # 무조건 100프레임씩 반복
    total_needed = base * N
    residual = total_t - total_needed  # 남은 프레임 수 (예: 1)
    if residual < 0:
        raise ValueError(f"total_t ({total_t}) is too small for base=100 * N={N}")
    # 각 chunk마다 반복할 수 (앞쪽에 residual만큼 1프레임씩 추가)
    repeat_counts = [base] * (N - 1) + [base + residual]  # only last gets extra
    # gate[:, i] → repeat_counts[i] 만큼 확장
    gate_chunks = [gate[:, i].unsqueeze(1).repeat(1, r) for i, r in enumerate(repeat_counts)]
    gate_expanded = torch.cat(gate_chunks, dim=1)  # shape: (B, total_t)
    # sanity check
    assert gate_expanded.shape[1] == total_t, f"Expanded gate shape mismatch: {gate_expanded.shape[1]} != {total_t}"
    return gate_expanded

class AudioLightningModuleMoe4(pl.LightningModule):
    def __init__(
        self,
    ...
    def get_mag(self, input):
        ...
        return mag
    def get_mag_from_spec(self, stft_spec):
        stft_spec = torch.view_as_real(stft_spec)
        mag = torch.sqrt(stft_spec.pow(2).sum(-1)+(1e-9)) #(1,321,401)
        return mag
    ...
    def prepare_discriminator_loss(self,mixtures, targets, est_sources,spec):
        ...
        chunked_est_sources_reorder_1 = chunk_waveform(est_sources[:,0])
        chunked_est_sources_reorder_2 = chunk_waveform(est_sources[:,1])
        ...
        chunked_est_sources_reorder_1 = chunk_spec(spec[:,0])
        chunked_est_sources_reorder_2 = chunk_spec(spec[:,1])
        chunked_est_sources_reorder = torch.stack((chunked_est_sources_reorder_1, chunked_est_sources_reorder_2), dim=1)  # (b*chunk,2,16000)
        
        est_mag1=self.get_mag_from_spec(chunked_est_sources_reorder_1)
        est_mag2=self.get_mag_from_spec(chunked_est_sources_reorder_2)
        return {
    ...
    def training_step(self, batch, batch_nb):
        ...
        optimizer.zero_grad()
        est_sources,est_sources_final,sep_output1,sep_output2,spec,spec_final,subband_spec = self(mixtures)
        B_for_gate, _, _, T_for_gate = sep_output1.shape
        loss,reorder_sources,batch_indices = self.loss_func["train"](est_sources, targets,return_ests=True)

        #2. discriminator with est_sources
        optimizer_d.zero_grad()
        chunked_mixtures = chunk_waveform(mixtures) #(4,36000)
        chunk_batch_size = chunked_mixtures.shape[0]
        one_labels = torch.ones(chunk_batch_size).to(mixtures.device, non_blocking=True)
        
        prepared=self.prepare_discriminator_loss(mixtures, targets, est_sources,spec)
        ...
        optimizer_d.step()
        #3. gate구하기 with est_sources + est_sources_gated + loss_refined
        with torch.no_grad():
            metric_g1 = self.discriminator(est_mag1) #
            metric_g2 = self.discriminator(est_mag2) #
            gate=self.sharpened_gate(metric_g1, metric_g2,T=0.7)
            gate=gate.view(mixtures.shape[0],-1) #(b,n)
            gate_expanded=expand_gate_to_time(gate,T_for_gate) #(b,401)
            gate_expanded = gate_expanded.view(B_for_gate,1,1,T_for_gate)  # (B, 1, 1, T)
        sep_output_gated = sep_output1 * gate_expanded + sep_output2 * (1 - gate_expanded)  # (B, C, F, T)
        est_sources_gated=self.audio_model.reconstruct_from_sep_output(mixtures,sep_output_gated,subband_spec)
        target_wav_reordered = torch.stack([targets[i, perm] for i, perm in enumerate(batch_indices)], dim=0)
        loss_refined= self.loss_func["train"].no_pit_forward(est_sources_gated, target_wav_reordered)
        
        chunked_est_sources1_final = chunk_spec(spec_final[:,0])
        chunked_est_sources2_final = chunk_spec(spec_final[:,1])
        est_mag1_final=self.get_mag_from_spec(chunked_est_sources1_final)
        est_mag2_final=self.get_mag_from_spec(chunked_est_sources2_final)
        with torch.no_grad():
        ...
        loss_gen_all = loss + loss_refined + loss_metric
    ...
    def validation_step(self, batch, batch_nb, dataloader_idx):
        ...
        if dataloader_idx == 0:
            ...
            est_sources_gated=self.audio_model.reconstruct_from_sep_output(mixtures,sep_output_gated,subband_spec)
            target_wav_reordered = torch.stack([targets[i, perm] for i, perm in enumerate(batch_indices)], dim=0)
            loss_refined= self.loss_func["val"].no_pit_forward(est_sources_gated, target_wav_reordered)
