...
def chunk_waveform(waveform, sr=24000, segment_secs=2.0, hop_ratio=0.5):
    """
    Args:
...
def main(args):
    ...
    if args.use_overlapadd:
        ...
        eval_save_dir = (
            f"{exp_dir}/result_chunk{os.path.basename(model_path).split('.')[0]}"
        )
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(test_set))):
            ...
            try:
                ...
                reordered_sources_original = reordered_sources.clone()
                ...
                #re_separate_sources=[]
                say=False
                for jj,chunk in enumerate(out_logits1):
                    if out_logits1[jj].item() < 0.5 or out_logits2[jj].item() < 0.5:
                        est_sources_chunk = model(chunked_mix[jj].unsqueeze(0),istest=True)
                        with torch.no_grad():
                            logit_re=sigmoid(discriminator(get_mag(est_sources_chunk[:,0])))+sigmoid(discriminator(get_mag(est_sources_chunk[:,1])))
                            if out_logits1[jj].item()+ out_logits2[jj].item()+1.0<logit_re[0].item():
                                print("✅✅",out_logits1[jj].item()+ out_logits2[jj].item(),"<", logit_re[0].item())
                                #reordered_sources[:,0],reordered_sources[:,1]랑 비교해서 est_sources_chunk[:,0] est_sources_chunk[:,1] 를 서로 cosine sim으로 
                                #비교해서 평균적으로 더 비슷한 쌍을 찾는다. 그리고 reordered_sources에 적절히 끼워넣는다.
                                est_chunk1 = est_sources_chunk[0, 0]  # shape: (T,)
                                est_chunk2 = est_sources_chunk[0, 1]
                                say=True
                                ref_chunk1 = reordered_sources[0, 0, jj * 48000 // 2 : jj * 48000 // 2 + 48000]
                                ref_chunk2 = reordered_sources[0, 1, jj * 48000 // 2 : jj * 48000 // 2 + 48000]
                                ...
                                sim_00 = cos_sim(est_chunk1, ref_chunk1)
                                sim_01 = cos_sim(est_chunk1, ref_chunk2)
                                sim_10 = cos_sim(est_chunk2, ref_chunk1)
                                sim_11 = cos_sim(est_chunk2, ref_chunk2)

                                sim_pair1 = (sim_00 + sim_11) / 2  # est1 to ref1, est2 to ref2
                                sim_pair2 = (sim_01 + sim_10) / 2  # est1 to ref2, est2 to ref1

                                if sim_pair2 > sim_pair1:
                                    # Swap
                                    reordered_sources[0, 0, jj * 48000 // 2 : jj * 48000 // 2 + 48000] = est_chunk2
                                    reordered_sources[0, 1, jj * 48000 // 2 : jj * 48000 // 2 + 48000] = est_chunk1
                                else:
                                    reordered_sources[0, 0, jj * 48000 // 2 : jj * 48000 // 2 + 48000] = est_chunk1
                                    reordered_sources[0, 1, jj * 48000 // 2 : jj * 48000 // 2 + 48000] = est_chunk2
                            #re_separate_sources.append(est_sources_chunk)    
                            else:
                                print("🔧🔧",out_logits1[jj].item()+ out_logits2[jj].item(),">", logit_re[0].item())
                ...
                est_sources_np_original= reordered_sources_original.squeeze(0).cpu().data.numpy() #* db2linear(-adjusted_gain, eps=0.0)
                ...
                utt_metrics_original = get_metrics(
                    mix_np,
                    sources_np,
                    est_sources_np_original,
                    sample_rate=args.sample_rate,
                    metrics_list=COMPUTE_METRICS,
                )
                ...
                utt_metrics['si_sdr_original'] = utt_metrics_original["si_sdr"]
                utt_metrics['sdr_original'] = utt_metrics_original["sdr"]
                #utt_metrics['sir_original'] = utt_metrics_original["sir"]
                series_list.append(pd.Series(utt_metrics))

                # Save some examples in a folder. Wav files and metrics as text.
                if True:#idx in save_idx:
                    local_save_dir = f"{ex_save_dir}/ex_{idx}_{say}/"
                    os.makedirs(local_save_dir, exist_ok=True)
                    sf.write(local_save_dir + "mixture.wav", mix_np, args.sample_rate)
                    print(local_save_dir + "mixture.wav")
                    ...
                    for src_idx, est_src in enumerate(est_sources_np_original):
                        sf.write(
                            f"{local_save_dir}/s{src_idx}_estimate_original.wav",
                            est_src,
                            args.sample_rate,
                        )
