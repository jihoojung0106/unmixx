...
def chunk_waveform(waveform, sr=24000, segment_secs=1.5, hop_ratio=0.25):
    """
    Args:
...
def main(args):
   ...
   # args.seq_dur=4
    ...
    ex_save_dir = f"{eval_save_dir}/examples_{args.singing_task}_24k_1"
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(test_set))):
            ...
            try:
                ...
                out_pred_est1= (out_logits1 >= 0.4).float() # threshold 0.5 기준으로 binary 예측
                out_pred_est2= (out_logits2 >= 0.4).float() # threshold 0.5 기준으로 binary 예측
                ...
                if out_pred_est_mean>=0.5 and num_zero_chunks ==1 :
                    flag=True
                    ...
                    for zero_idx in zero_indices:
                        ...
                        def cos_sim(a, b):
                            return F.cosine_similarity(
                                ...
                                F.normalize(b.unsqueeze(0), dim=-1)
                            ).item()
