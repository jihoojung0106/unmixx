...
import os
import glob
csv_paths= glob.glob("duet_svs/CSD/**/*.csv",recursive=True)
for csv_path in csv_paths:
    #csv_path="en002b.csv"
    # ----------- 2. CSV 로딩 및 전처리 -----------
    df = pd.read_csv(csv_path)
    df = df.rename(columns={"start": "start_time_sec", "end": "end_time_sec", "syllable": "lyric"})
    ...
    output_json_path=csv_path.replace("/csv/","/wav/").replace(".csv", "_unison.json")
    if len(segment)==0:
