...
MAX_FREQ_IDX = 87
def contour_bin_to_hz(bin_index: float) -> float:
    """
    Convert contour bin index to frequency in Hz.

    Args:
        bin_index: index in the contour matrix (float or int)

    Returns:
        Corresponding frequency in Hz
    """
    return ANNOTATIONS_BASE_FREQUENCY * 2 ** (bin_index / (12.0 * CONTOURS_BINS_PER_SEMITONE))
# def contour_bin_to_hz(bin_index: float) -> float:
#     return 27.5 * 2 ** (bin_index / (12.0 * 3))
