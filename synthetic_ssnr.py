...
class MyMedleyVox(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mixture_path=mixture_path.replace(self.root_dir, "duet_svs/MedleyVox")
        self.mixture_path = mixture_path
        sources_path_list = glob.glob(f"{self.total_segments_list[idx]}/gt/*.wav")
        sources_cross_path_list = [x for x in sources_path_list if "_cross.wav" in x]
        sources_path_list=[x for x in sources_path_list if "_cross.wav" not in x]  # Exclude cross-talk files
...
def main(args):
    ...
    eval_save_dir = args.test_dir #Experiments/checkpoint/melt6/epoch=21.ckpt
